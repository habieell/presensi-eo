...
import {
  CalendarDays, Camera, ClipboardList, Coins, Home,
  LayoutDashboard, Users, Wallet, FileClock, ClockIcon,
} from 'lucide-react';
...
export default function BottomNavbar({ isAdmin }: { isAdmin: boolean }) {
  ...
  const activeIndex = items.findIndex((it) =>
    it.to === '/admin' ? location.pathname === '/admin' : location.pathname.startsWith(it.to),
  );
    ...
    <nav className="bottom-nav">
      <div className="max-w-6xl mx-auto flex items-stretch justify-between gap-1 relative">
        {items.map((item, idx) => {
          ...
          return <NavItemAnimated key={item.to} item={item} isActive={isActive} />;
        })}
      </div>
    </nav>
  );
...
function NavItemAnimated({ item, isActive }: { item: NavItem; isActive: boolean }) {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {isActive && (
          <motion.span
            ...
            transition={SPRING}
            style={{
              boxShadow: '0 8px 24px rgba(251, 191, 36, 0.45), inset 0 1px 0 rgba(255,255,255,0.30)',
            }}
          />
        )}
      </AnimatePresence>
      ...
      <motion.span
        ...
        animate={isActive ? { scale: [1, 1.25, 1], rotate: [0, -8, 0] } : { scale: 1 }}
        transition={isActive ? { duration: 0.45, ease: 'easeOut' } : { duration: 0.2 }}
      >
        <Icon
      ...
      <motion.span
        ...
        initial={false}
        animate={isActive ? { y: 0, opacity: 1, fontWeight: 800 } : { y: 0, opacity: 0.85, fontWeight: 600 }}
      ...
      <motion.span
        ...
        initial={{ opacity: 0 }}
        whileHover={!isActive ? { opacity: 1, backgroundColor: 'rgba(251, 191, 36, 0.06)' } : {}}
